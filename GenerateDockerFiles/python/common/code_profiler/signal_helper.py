...
class SignalHelper():
    ...
    def _is_signal_usr_signal_handlers_used(self, signal_to_test):
        ...
        status = signal_handler is None            or \
                 self._is_default_signal_handler(signal_handler) or \
                 self._is_ignore_signal_handler(signal_handler)  or \
                 self._is_gunicorn_logfile_signal_handler(signal_handler) 
