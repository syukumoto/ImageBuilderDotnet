...
if [[ $response == "200" ]]; then
    ...
    if [[ $CDN_ENDPOINT ]]; then
        if [[ "$1" == "BLOB_CDN" ]] && [ ! $(grep "BLOB_CDN_CONFIGURATION_COMPLETE" $WORDPRESS_LOCK_FILE) ] \
        && wp w3-total-cache option set cdn.azure.cname $CDN_ENDPOINT --type=array --path=$WORDPRESS_HOME --allow-root; then
            echo "BLOB_CDN_CONFIGURATION_COMPLETE" >> $WORDPRESS_LOCK_FILE
            service atd stop
            redis-cli flushall
        elif [[ "$1" == "CDN" ]] && [ ! $(grep "CDN_CONFIGURATION_COMPLETE" $WORDPRESS_LOCK_FILE) ] \
        ...
        && wp w3-total-cache option set cdn.mirror.domain $CDN_ENDPOINT --type=array --path=$WORDPRESS_HOME --allow-root; then
    	    ...
    	    service atd start
            echo "bash /usr/local/bin/w3tc_cdn_config.sh $1" | at now +5 minutes
        fi
    fi
else
    service atd start
